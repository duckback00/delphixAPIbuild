...
#
# Parse JSON Object and Get Results ...
#
$a = $results.result
...
switch (${ACTION}) {
   ...
   "create" { 
      ...
      if ( "${ACTION}" -eq "create" -and "${GROUP_REFERENCE}" -eq "" ) {
         ...
         $json = "{ ""type"": ""Group"" ,""name"": ""${DELPHIX_GRP}"" } "

         #Write-Output "${nl}Create Group API ...${nl}"
         $results = Invoke-RestMethod -Method POST -body "${json}" -ContentType "application/json" -WebSession $session -URI "${BaseURL}/group"
         $status = ParseStatus $results "${ignore}"
         Write-Output "group ${ACTION} API Results: ${results}"

         #
         # Results JSON Object and Get Results Status ...
         #
         $a=$results.result
...
"@
         ...
         $results = Invoke-RestMethod -Method POST -body "${json}" -ContentType "application/json" -WebSession $session -URI "${BaseURL}/group/${GROUP_REFERENCE}/${ACTION}"
         $status = ParseStatus $results "${ignore}"
