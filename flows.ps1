...
$results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/timeflow?database=${CONTAINER_REFERENCE}"
$status = ParseStatus ${results} "${ignore}"
...
$results = Invoke-RestMethod -URI "${BaseURL}/timeflow/${FLOW_REF}/timeflowRanges" -WebSession $session -Method Post -Body $json -ContentType 'application/json'
$status = ParseStatus $results "${ignore}"
...
$results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/timeflow/${FLOW_REF}"
$status = ParseStatus ${results} "${ignore}"
...
$results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/snapshot/${SYNC_REF}"
$status = ParseStatus ${results} "${ignore}"
#Write-Output "results: $results"
#$o = ConvertFrom-Json $results
$a = $results.result
...
$SPACE = Invoke-RestMethod -URI "${BaseURL}/snapshot/space" -WebSession $session -Method Post -Body $json -ContentType 'application/json'
$SPACE.error
$SPACE.error.details
